...
public class UserService {
	...
	public AuthResponse sendResetLink(ForgotPasswordRequest request) {
		...
		if (userOpt.isPresent()) {
			...
			String plainOtp = otpService.generateOtp(user);
			mailService.sendOtp(user.getEmail(), plainOtp);
		}
	...
	public AuthResponse verifyOtp(VerifyOtpRequest request) {
		User user = userRepository.findByEmail(request.getEmail())
				.orElseThrow(() -> new ValidationException("Invalid email or OTP"));
	...
	public AuthResponse resetPassword(ResetPasswordRequest request) {
		...
		if (!otpService.isEmailOtpVerified(user)) {
			throw new ValidationException("OTP not verified");
		}
